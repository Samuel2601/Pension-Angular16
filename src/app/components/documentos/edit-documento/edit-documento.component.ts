...
export class EditDocumentoComponent implements OnInit {
	...
	listar_etiquetas_documento() {
		...
		this._adminService.listar_etiquetas_documento_admin(this.id, this.token).subscribe((response) => {
	...
	eliminar_etiqueta(id: any) {
		this.load_etiquetas = true;
		this._adminService.eliminar_etiqueta_documento_admin(id, this.token).subscribe((response) => {
			this.listar_etiquetas_documento();
			this.load_etiquetas = false;
		});
	}
	...
	agregar_etiqueta() {
		...
		this.load_etiquetas = true;
		this._adminService.agregar_etiqueta_documento_admin(data, this.token).subscribe((response) => {
			this.listar_etiquetas_documento();
			this.load_etiquetas = false;
		});
	}
	...
	actualizar(actualizarForm: any) {
		if (actualizarForm.valid) {
			...
			if (!this.documento.precio_antes_soles || this.documento.precio_antes_soles == undefined) {
				this.documento.precio_antes_soles = 0;
			}

			if (!this.documento.precio_antes_dolares || this.documento.precio_antes_dolares == undefined) {
